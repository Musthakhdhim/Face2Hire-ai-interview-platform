...
import { Loader2, Mail, Phone, Calendar, FileText, User } from 'lucide-react';
...
export default function AdminUserDetailPage() {
    ...
    useEffect(() => {
        const fetchDetail = async () => {
            try {
                ...
                console.log('User detail:', data); // Debug log
                setUserDetail(data);
            } catch (error) {
                console.error(error);
    ...
    // Fix: The backend sends "active" (boolean) for isActive due to JavaBean naming
                        ...
                        <CardContent>
                            <div className="grid md:grid-cols-5 gap-4 mb-6">
                                <div className="p-4 bg-blue-50 rounded-lg text-center">
                                    <div className="text-3xl font-bold text-blue-600">{userDetail.interviewStats.totalCompletedInterviews}</div>
                                    <div className="text-sm text-gray-600">Completed Interviews</div>
                                </div>
                                <div className="p-4 bg-green-50 rounded-lg text-center">
                                    <div className="text-3xl font-bold text-green-600">{Math.round(userDetail.interviewStats.avgOverallScore)}%</div>
                                    <div className="text-sm text-gray-600">Avg Overall</div>
                                </div>
                                <div className="p-4 bg-purple-50 rounded-lg text-center">
                                    <div className="text-3xl font-bold text-purple-600">{Math.round(userDetail.interviewStats.avgCommunicationScore)}%</div>
                                    <div className="text-sm text-gray-600">Communication</div>
                                </div>
                                <div className="p-4 bg-amber-50 rounded-lg text-center">
                                    <div className="text-3xl font-bold text-amber-600">{Math.round(userDetail.interviewStats.avgTechnicalScore)}%</div>
                                    <div className="text-sm text-gray-600">Technical</div>
                                </div>
                                <div className="p-4 bg-indigo-50 rounded-lg text-center">
                                    <div className="text-3xl font-bold text-indigo-600">{Math.round(userDetail.interviewStats.avgConfidenceScore)}%</div>
                                    <div className="text-sm text-gray-600">Confidence</div>
                                </div>
                            </div>
                            {userDetail.interviewStats.totalCompletedInterviews === 0 && (
